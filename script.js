...
window.loadBuiltInData = function() {
    ...
    // === 【修改点 3-2】规则数据 (词条) - 整合你的内容并美化 ===
    ruleTerms = [
    // 游戏术语词条
    {
        title: "基础牌型",
        content: "游戏包含三类核心牌型：\n\n" +
        "• **状态**：仅自己回合发动，分减益/增益/中性。减益放对手场、增益放自己场，中性随意。发动后置于指定角色场上持续生效（右下角为持续回合数），角色回合结束时结算，多状态卡结算触发连锁，遵循「后发先至」。每次角色回合结束，持续回合数减1。\n\n" +
        "• **法术**：分【吟唱】【速攻】【反制】三类，同一时序发动的法术构成连锁，结算遵循「后发先至」：\n  - 【吟唱】：仅自己回合发动，无法连锁其他法术；\n  - 【速攻】：可在对方回合发动，无法连锁【反制】；\n  - 【反制】：可在对方回合发动，可连锁任意法术。\n\n" +
        "• **装备**：仅自己回合使用，分【武器】【宝具】：\n  - 【武器】：初始1个槽位，有耐久度，每次攻击减1；\n  - 【宝具】：初始2个槽位，有全局属性。宝具槽满时，玩家获得【辎重】。"
    },
    {
        title: "核心概念",
        content: "• **牌堆**：玩家自定义卡组，固定30张。同名卡≤2张，传说卡（7字/含「·」）仅能携带1张。\n\n" +
        "• **牌河**：额外卡组，上限5张，可随时查看但不可改顺序（初始顺序可自定义）。每次自动升阶时，可放弃升阶改为从牌河抽1张卡。\n\n" +
        "• **灵能**：限制卡牌使用的资源，每回合上限2点，自己回合开始时回满。使用卡牌通常消耗1点。\n\n" +
        "• **阶数**：限制强力卡牌的手段，法术/装备/状态等阶独立计算，无法使用高于自身当前等阶的卡牌。己方每回合开始选1类卡牌升1阶；后手第3回合额外获得1次升阶机会。\n  示例：当前等阶「法术2/装备2/状态1」，只能用2阶装备/法术、1阶状态，下次升阶仅能到「2/2/2」（不可跳升）。\n\n" +
        "• **场上**：放置状态卡、装备卡的区域，每个玩家场上最多8张卡，超出无法放置。"
    },
    // 卡牌效果词条
    {
        title: "卡牌效果词条",
        content: "• **辎重**：无法装备新宝具，手牌上限-1。\n" +
        "• **禁制**：此卡需满足额外条件才能使用（注：若含禁制的卡牌发动后被无效，不回溯已消耗的资源）。\n" +
        "• **引渡：x**：查看牌河，将x张卡加入手牌。\n" +
        "• **和鸣**：此卡在牌河时，激活额外效果。\n" +
        "• **占卜：x**：查看牌堆顶x张牌，可调整顺序或移至牌底。\n" +
        "• **运势：x**：投掷1次骰子，点数≥x则触发额外效果。\n" +
        "• **余烬**：此卡在墓地时，获得额外效果。\n" +
        "• **易伤：x**：下一次受到的伤害提高x点，回合结束时清空。\n" +
        "• **瞬发**：每回合第一张此类型卡牌不消耗灵能。\n" +
        "• **压轴**：自己无灵能时，使用此卡不消耗灵能（每回合限1次）。\n" +
        "• **贯通**：造成的伤害无视免疫、减免、护甲效果。\n" +
        "• **连击**：打出此卡前使用过其他卡牌，则触发额外效果。"
    }
];
